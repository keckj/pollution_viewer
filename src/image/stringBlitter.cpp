...
Image StringBlitter::generateTextImageGraylevel(const std::string &str) {
    ...
    for (unsigned int i = 0; i < str.size(); i++) {
        ...
        CHECK_FREETYPE_ERROR(FT_Load_Glyph(face, index, FT_LOAD_DEFAULT));
        CHECK_FREETYPE_ERROR(FT_Render_Glyph(slot, FT_RENDER_MODE_NORMAL));
        blitCharacter(image, x+slot->bitmap_left, imgInfo.maxBearingY - slot->bitmap_top, bitmap);
        x += slot->advance.x >> 6;
    }
...
Image StringBlitter::generateTextImageRGBA(const std::string &str, const ColorRGBA &color) {
    ...
    for (int j = 0; j < rgbaImg.height; j++) {
        for (int i = 0; i < rgbaImg.width; i++) {
...
void StringBlitter::blitCharacter(const Image &image, int x, int y, const FT_Bitmap &bitmap) {
    ...
    for (int j = 0; j < bitmap.rows; j++) {
        for (int i = 0; i < bitmap.width; i++) {
            image.data[(y+j)*image.width+(x+i)] = glyphData[j*bitmap.pitch +i];
        }
    }
}
